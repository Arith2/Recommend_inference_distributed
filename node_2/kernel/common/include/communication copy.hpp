...
void openConnections( int useConn, int baseIpAddress, int basePort, hls::stream<pkt64>& m_axis_tcp_open_connection, hls::stream<pkt128>& s_axis_tcp_open_status, ap_uint<16>* sessionID, hls::stream<ap_uint<1> >& openConnection_flag)
{
...
// #pragma HLS dataflow disable_start_propagation
    ...
    //  }
     ...
     pkt64 openConnection_pkt;
     openStatus status;
     bool openConnectionFlag = false;

     do{
        if (!openConnection_flag.empty())
        {
            openConnectionFlag = true;
...
#pragma HLS INTERFACE ap_stable port=expectedTxByteCnt
     ...
     do{
          ...
          {
               if (!s_axis_tcp_tx_status.empty() & !s_data_in.empty())
               {
                    pkt64 txStatus_pkt = s_axis_tcp_tx_status.read();
                    resp.sessionID = txStatus_pkt.data(15,0);
                    resp.length = txStatus_pkt.data(31,16);
