...
#pragma HLS INTERFACE ap_stable port=expectedTxByteCnt
     ...
     ap_uint<16> cur_sessionID = 0;

     do{
          pkt32 tx_meta_pkt;
          appTxRsp resp;

        //   if (first_round & !openConnection_flag.empty() & !s_data_in.empty())
        if (first_round & !sessionID_stream.empty() & !s_data_in.empty())
          { 
               cur_sessionID = sessionID_stream.read();
               tx_meta_pkt.data(15,0) = cur_sessionID;//sessionID[currentSessionIndex];
               tx_meta_pkt.data(31,16) = pkgWordCount*(512/8);
               m_axis_tcp_tx_meta.write(tx_meta_pkt);
               ...
               if (!s_axis_tcp_tx_status.empty())
               {
                    ...
                    if (resp.error == 0)
                    {
                        ...
                        //  }
                         ...
                         {
                            //   tx_meta_pkt.data(15,0) = sessionID[currentSessionIndex];
                              tx_meta_pkt.data(15,0) = cur_sessionID;
                              if (sentByteCnt + pkgWordCount*64 < expectedTxByteCnt )
                              {
                                  tx_meta_pkt.data(31,16) = pkgWordCount*(512/8);
                                  currentPkgWordCnt = pkgWordCount;
...
void recvData_handshake(ap_uint<64> expRxBytePerSession_0,
               ap_uint<64> expRxBytePerSession_1, 
               int basePort_1,
               hls::stream<pkt128>& s_axis_tcp_notification, 
               hls::stream<pkt32>& m_axis_tcp_read_pkg,
               hls::stream<ap_uint<32> >& nextRxPacketLength,
               hls::stream<ap_uint<16> >& sessionID_stream)
{
     ap_uint<64> rxByteCnt = 0;
     ap_uint<64> rxByte_total = expRxBytePerSession_0 + expRxBytePerSession_1;
     bool first_round = true;
     ...
     do{
          if (!s_axis_tcp_notification.empty())
          {
               ...
               if (length!=0)
               {
                    ...
                    if (dstPort==basePort_1) {
                        if (first_round) {
                            first_round = false;
                            sessionID_stream.write(sessionID);
                        }
                        ap_uint<32> tmp_data = 0;
                        tmp_data(15, 0) = length;
                        tmp_data(31, 16) = 1;
                        nextRxPacketLength.write(tmp_data);
                    }
                    else {
                        ap_uint<32> tmp_data = 0;
                        tmp_data(15, 0) = length;
...
void recvData_consumeData(ap_uint<64> expRxBytePerSession_0,
               ap_uint<64> expRxBytePerSession_1, 
               hls::stream<pkt16>& s_axis_tcp_rx_meta, 
               hls::stream<pkt512>& s_axis_tcp_rx_data,
               hls::stream<ap_uint<32> >& nextRxPacketLength,
               hls::stream<ap_uint<512> >& s_data_out )
{
     ...
     ap_uint<64> rxByte_total = expRxBytePerSession_0 + expRxBytePerSession_1;
     ap_uint<16> length = 0;
     ap_uint<16> listen_port = 0;

     do{
          if (!s_axis_tcp_rx_meta.empty() & !nextRxPacketLength.empty())
          {
               s_axis_tcp_rx_meta.read();
               ap_uint<32> tmp_data = nextRxPacketLength.read();
               length = tmp_data(15, 0);
               listen_port = tmp_data(31, 16);
               bool lastWord = false;
               do{
                    pkt512 rx_data = s_axis_tcp_rx_data.read();
                    lastWord = rx_data.last;
...
void recvData(ap_uint<64> expRxBytePerSession_0, 
               ap_uint<64> expRxBytePerSession_1, 
               int basePort_1,
               hls::stream<ap_uint<512> >& s_data_out,
               hls::stream<pkt128>& s_axis_tcp_notification, 
...
#pragma HLS dataflow disable_start_propagation
     ...
     recvData_handshake(expRxBytePerSession_0, 
               expRxBytePerSession_1,
               basePort_1,
               s_axis_tcp_notification, 
               m_axis_tcp_read_pkg,
               nextRxPacketLength,
     ...
     recvData_consumeData(expRxBytePerSession_0, 
               expRxBytePerSession_1,
               s_axis_tcp_rx_meta, 
               s_axis_tcp_rx_data,
               nextRxPacketLength,
