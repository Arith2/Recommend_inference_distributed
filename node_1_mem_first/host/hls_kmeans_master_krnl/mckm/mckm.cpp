...
void shuffle_object (float * objects, int numCoords, int numObjs, float* shuffled_objects)
{
    vector<int> index;
    for (int i = 0; i < numObjs; ++i)
    {
    ...
    shuffle (index.begin(), index.end(), std::default_random_engine(0));
...
void initial_centroids(int numClusters, int numCoords, int numObjs, float* cluster, float* objects) // numCoords是维度数，numObjs是数据总数
{
    for (int i=0; i<numClusters; i++) {
