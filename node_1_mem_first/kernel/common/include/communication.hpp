...
void openConnections(int useConn, ap_uint<32>* baseIpAddress, ap_uint<16>* basePort, hls::stream<pkt64>& m_axis_tcp_open_connection, hls::stream<pkt128>& s_axis_tcp_open_status, hls::stream<ap_uint<16> >& sessionID_stream)
{
     int numOpenedCon = 0;
     pkt64 openConnection_pkt;
     ...
     #pragma HLS PIPELINE II=1
          ...
          status.sessionID = open_status_pkt.data(15,0);
          status.success = open_status_pkt.data(16,16);
          status.ip = open_status_pkt.data(55, 24);
          status.port = open_status_pkt.data(71, 56);

          if (status.success)
          {
               if (status.port==basePort[0]) {
                    sessionID_array[0] = status.sessionID;
               }
               else {
                    sessionID_array[1] = status.sessionID;
               }
            //    sessionID[numOpenedCon] = status.sessionID;
            //    sessionID_stream.write(status.sessionID);
               numOpenedCon++;
               std::cout << "Connection successfully opened." << std::endl;
          }
     }
     ...
     for (int i = 0; i < useConn; i++) {
     #pragma HLS PIPELINE II=1   
        sessionID_stream.write(sessionID_array[i]);
     }
}
...
void sendData(hls::stream<pkt32>& m_axis_tcp_tx_meta, 
               hls::stream<pkt512>& m_axis_tcp_tx_data, 
               hls::stream<pkt64>& s_axis_tcp_tx_status,
            //    hls::stream<ap_uint<512> >& s_data_in_0,
               hls::stream<ap_uint<512> >& s_data_in,
               hls::stream<ap_uint<16> >& sessionID_stream,
               int useConn,
               ap_uint<64> expectedTxByteCnt_0, 
               ap_uint<64> expectedTxByteCnt_1, 
               int pkgWordCount
                )
{
#pragma HLS INTERFACE ap_stable port=pkgWordCount
#pragma HLS INTERFACE ap_stable port=useConn
#pragma HLS INTERFACE ap_stable port=expectedTxByteCnt_0
#pragma HLS INTERFACE ap_stable port=expectedTxByteCnt_1

     bool first_round = true;
     ap_uint<64> sentByteCnt_0 = 0;
     ap_uint<64> sentByteCnt_1 = 0;
     int currentPkgWordCnt = pkgWordCount;
     int currentSessionIndex = 0;
     ap_uint<16> sessionID_array [8];
     ...
     do{
        ...
        //   if (first_round & !sessionID_stream.empty() & !s_data_in_0.empty() & !s_data_in.empty())
        if (first_round & !sessionID_stream.empty() & !s_data_in.empty())
          {
               for (int i = 0; i < useConn; ++i)
               {
                    #pragma HLS PIPELINE II=1
                    sessionID_array[i] = sessionID_stream.read();
                    tx_meta_pkt.data(15,0) = sessionID_array[i];
                    tx_meta_pkt.data(31,16) = pkgWordCount*(512/8);
                    m_axis_tcp_tx_meta.write(tx_meta_pkt);
               }
          ...
          else
          {
               if (!s_axis_tcp_tx_status.empty())
               {
                    ...
                    if (resp.error == 0)
                    {
                        if (resp.sessionID == sessionID_array[0]) {
                            sentByteCnt_0 = sentByteCnt_0 + resp.length;
                            // currentPkgWordCnt = pkgWordCount;
                            for (int j = 0; j < currentPkgWordCnt; ++j)
                            {
                                #pragma HLS PIPELINE II=1
                                // ap_uint<512> s_data = s_data_in_0.read();
                                pkt512 currWord;
                                for (int i = 0; i < (512/64); i++) 
                                {
                                     #pragma HLS UNROLL
                                     currWord.data(i*64+63, i*64) = 0x000000000000ffff;//s_data(i*64+63, i*64);
                                     currWord.keep(i*8+7, i*8) = 0xff;
                                }
                                currWord.last = (j == currentPkgWordCnt-1);
                                m_axis_tcp_tx_data.write(currWord);
                        ...
                        // the next node with continuous inputs
                        else {
                            sentByteCnt_1 = sentByteCnt_1 + resp.length;
                            // currentPkgWordCnt = pkgWordCount;
                            if (sentByteCnt_1 < expectedTxByteCnt_1)
                            {
                                tx_meta_pkt.data(15,0) = resp.sessionID;
                                tx_meta_pkt.data(31,16) = pkgWordCount*(512/8);
                                m_axis_tcp_tx_meta.write(tx_meta_pkt);
                        ...
                        run_flag = (sentByteCnt_0 < expectedTxByteCnt_0) | (sentByteCnt_1 < expectedTxByteCnt_1);
                    }
                    else
                    {
                         //Check if connection  was torn down
...
//           if (first_round & !s_data_in.empty())
//           {
//                tx_meta_pkt.data(15,0) = sessionID[currentSessionIndex];
//                tx_meta_pkt.data(31,16) = pkgWordCount*(512/8);
//                m_axis_tcp_tx_meta.write(tx_meta_pkt);
...
//           if (first_round & !s_data_in.empty() & !openConnection_flag.empty())
//           {
//                tx_meta_pkt.data(15,0) = sessionID[currentSessionIndex];
//                tx_meta_pkt.data(31,16) = pkgWordCount*(512/8);
//                m_axis_tcp_tx_meta.write(tx_meta_pkt);
...
//             syn_signal = true;
//           }
//           else if (!s_data_in.empty()) {
//             if (first_round | syn_signal) {
//                 if (sentByteCnt + pkgWordCount*64 < expectedTxByteCnt) {
//                     currentPkgWordCnt = pkgWordCount;
//                 }
//                 else {
...
//                 tx_meta_pkt.data(15,0) = sessionID[currentSessionIndex];
//                 tx_meta_pkt.data(31,16) = currentPkgWordCnt*(512/8);
//                 m_axis_tcp_tx_meta.write(tx_meta_pkt);
...
void port_status_handler(int basePort, int useConn, hls::stream<pkt8>& s_axis_tcp_port_status, int& success_open_port, hls::stream<ap_uint<1> >& openConnection_flag)
{
     int num_open_port = 0;
     for (int i = 0; i < useConn; ++i)
     {
